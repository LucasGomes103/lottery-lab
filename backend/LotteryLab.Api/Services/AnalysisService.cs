...
public sealed class AnalysisService(Db db) {
  public async Task<ForecastResponse> Forecast(string bank,string time,int windowDays,int top) {
    using var c=db.Open();
    var rows=(await c.QueryAsync<dynamic>(@"select e.extraction_date, r.dezena from results r join extractions e on e.id=r.extraction_id where e.bank=@bank and e.extraction_time=@time::time and e.extraction_date >= current_date-@windowDays and r.position between 1 and 10 order by e.extraction_date", new{bank,time,windowDays})).ToList();
  ...
  public async Task<object> Backtest(string bank,string time,int windowDays,int top) {
    using var c=db.Open();
    var rows=(await c.QueryAsync<(DateTime date,string dezena)>(@"select e.extraction_date as date, r.dezena from results r join extractions e on e.id=r.extraction_id where e.bank=@bank and e.extraction_time=@time::time and r.position between 1 and 10 order by e.extraction_date",new{bank,time})).ToList();
